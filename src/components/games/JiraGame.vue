<template>
  ...
  <div class="game">
    <div class="column todo" @dragover.prevent @drop="onDrop('todo')">
      <div class="stage">Нужно сделать</div>
      <div class="task" v-for="task in tasks" :key="task.id" draggable="true" @dragstart="onDragStart(task.id)">
        {{ task.name }}
      </div>
    </div>
    <div class="column inProgress" @dragover.prevent @drop="onDrop('inProgress')">
      <div class="stage">В работе</div>
      <div class="task" v-for="task in inProgressTasks" :key="task.id" draggable="true"
        @dragstart="onDragStart(task.id)">
        {{ task.name }}
      </div>
    </div>
    <div class="column done" @dragover.prevent @drop="onDrop('done')">
      <div class="stage">Демонстрация</div>
      <div class="task" v-for="task in doneTasks" :key="task.id" draggable="true" @dragstart="onDragStart(task.id)">
        {{ task.name }}
      </div>
    </div>
    <div v-if="isToDoEmpty" class="congratulations">
